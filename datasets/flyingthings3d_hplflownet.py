...
class FT3D(Dataset):
    ...
    def get_file_list(self):
        ...
        if self.mode == "train" or self.mode == "val":
            ...
            ind_val = set(np.linspace(0, 19639, 200).astype("int"))
            if self.full:
                ind_train = set(np.arange(19640).astype("int"))
            # ind_train = ind_all - ind_val
            else:
                ind_train = set(np.arange(0,19640,2).astype("int"))
            # assert (len(ind_train.intersection(ind_val)) == 0
