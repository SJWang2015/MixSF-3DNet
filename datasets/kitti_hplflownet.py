...
class Kitti(SceneFlowDataset):
    def __init__(self, root_dir, nb_points, mode):
        ...
        assert len(self.paths) == 142, "Problem with size of training set"
        ind_val = set(np.sort(np.linspace(0, 141, 141).astype("int")))

        ind_all = set(np.sort(np.arange(142).astype("int")))
        ind_train = ind_all - ind_val
        assert (
                len(ind_train.intersection(ind_val)) == 0
        ), "Train / Val not split properly"
        filenames = np.sort(self.paths)
        if self.mode == "train":
            self.paths = filenames[list(ind_train)]
        else:
            self.paths = filenames[list(ind_val)]
    ...
    def load_sequence(self, idx):
        ...
        # ground_truth = [ground_truth[i][loc] for i in range(2)]
        # loc = sequence[1][:, 2] < 35
