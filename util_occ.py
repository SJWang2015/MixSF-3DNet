...
#         else:
#             pc1w = pc1
#         inter_feats_bwd, _, _ = self.get_patch_features(pc2, pc1w, channelMixMLPs01=self.channelMixMLPs01, channelMixMLPs02=self.channelMixMLPs02, channelMixMLPs03=self.channelMixMLPs03, mask=mask, use_cross_att=True)
...
#         # grouped_point_to_patch_cost = inter_feats[intra_knn_idx.flatten(), :].view(N, self.nsample, -1)#index_points_group(inter_feats, intra_knn_idx) # N, nsample, C
#         # new_feats_v = self.channelMixMLPs03(new_feats)  # (n, in_planes) -> (n, k)
        
#         # patch_to_patch_cost = mask_groupped * intra_groupped_feats * grouped_point_to_patch_cost
        
#         # patch_to_patch_cost = patch_to_patch_cost.reshape(N, self.nsample, self.out_channel)
#         # if mask != None:
#         #     # mask = mask > 0.5
#         #     mask_groupped = mask[intra_knn_idx.flatten(), :].view(N, self.nsample, -1).repeat(1,1,self.out_channel)
#         #     patch_to_patch_cost = (mask_groupped * w * grouped_point_to_patch_cost).sum(1)
#         # else:
#         #     # patch_to_patch_cost = (intra_groupped_feats * grouped_point_to_patch_cost).sum(1)
...
class CostVolumeNetVanilla(nn.Module):
    def __init__(self, nsample, sub_nsample, in_channel, mid_channel, share_channel, out_channel, use_mask=False, use_leaky=True,
        intraLayer='PointMixerIntraSetLayer',
        interLayer='PointMixerInterSetLayer',
        transup='SymmetricTransitionUpBlock', 
        transdown='TransitionDownBlock'):
        ...
        self.share_channel = share_channel
        self.out_channel = out_channel
        self.sub_nsample = sub_nsample

        # self.mlp_convs = nn.ModuleList()
        # self.mlp_bns = nn.ModuleList()
        # # self.mlp2_convs = nn.ModuleList()
        # last_channel = in_channel*2+3
        # mlp = [mid_channel, out_channel]
        # for out_channel in mlp:
        #     self.mlp_convs.append(nn.Conv2d(last_channel, out_channel, 1, bias = False))
        #     self.mlp_bns.append(nn.BatchNorm2d(out_channel))
        #     last_channel = out_channel
        ...
        self.linear_p2 = nn.Sequential( # input.shape = [N, K, C]
            nn.Linear(3, out_channel//2, bias=False),
            nn.Sequential(
                Rearrange('n k c -> n c k'),
                nn.BatchNorm1d(out_channel//2),
    ...
    def get_patch_features(self, pc1, pc2, dpos_diff_knn, channelMixMLPs01, channelMixMLPs02, channelMixMLPs03, linear_p, sf=None, mask=None, use_cross_att=True, reverse=False, use_bwd=True):
        '''
        Input:
        (dense) pc1: [pos:[n, 3]; feats:[n, c]; cnt_num:[b*n]]
        (sparse) pc2: [pos:[m, 3]; feats:[m, c]; cnt_num:[b2*n]]
        ...
        p1, feats1, cnt_num1 = pc1
        p2, feats2, cnt_num2 = pc2 
        # dp2, dfeats2, d_cnt_num2 = dpc2 

        if use_cross_att and sf != None:
            if reverse:
                pos_feats_knn, knn_idx = pointops.queryandgroup(
                self.nsample, p1+sf, p2, feats1, None, cnt_num1, cnt_num2, use_xyz=True, return_idx=True)  # (n, k, 3+c)
            else:
                pos_feats_knn, knn_idx = pointops.queryandgroup(
                self.nsample, p2, p1+sf, feats2, None, cnt_num2, cnt_num1, use_xyz=True, return_idx=True)  # (n, k, 3+c)
        else:
            if reverse:
                pos_feats_knn, knn_idx = pointops.queryandgroup(
                self.nsample, p1, p2, feats1, None, cnt_num1, cnt_num2, use_xyz=True, return_idx=True)  # (n, k, 3+c)
            else:
                pos_feats_knn, knn_idx = pointops.queryandgroup(
                self.nsample, p2, p1, feats2, None, cnt_num2, cnt_num1, use_xyz=True, return_idx=True)  # (n, k, 3+c)
        
        if reverse:
            pos_diff = p1[knn_idx.view(-1), :].view(p2.shape[0], self.nsample, 3) - p2.unsqueeze(1) #pos_feats_knn[:, :, 0:3]
            feats2_groupped = feats2.view(len(p2), 1, -1).repeat(1, self.nsample, 1)
            # pos_feats_knn[:, :, 0:3] = pos_diff
            new_feats = torch.cat([pos_feats_knn, feats2_groupped], dim=-1)
        else:
        ...
        if dpos_diff_knn != None:
            dpos_diff_knn = dpos_diff_knn + pos_diff.unsqueeze(2)
            pos_diff = dpos_diff_knn.reshape(len(p1), -1, 3)
            sub_nsmaple = self.sub_nsample
            new_feats = new_feats.unsqueeze(2).repeat(1,1,sub_nsmaple,1).reshape(len(p1), -1, new_feats.shape[-1])
        else:
            sub_nsmaple = 1

        new_feats[:, :, 0:3] = pos_diff
        p_embed = linear_p(pos_diff) # (n, k, out_planes)
        p_embed_shrink = self.shrink_p(p_embed) # (n, k, k)
        ...
        w = self.softmax(energy)
        if use_cross_att: # and sf != None
            new_feats_v = channelMixMLPs03(new_feats) + p_embed # (n, in_planes) -> (n, k)
            n, nsample, out_planes = new_feats_v.shape
            
            with torch.no_grad():
                knn_idx_flatten = rearrange(knn_idx, 'm k -> (m k)')

            if mask != None and reverse:
                # backward cost
                mask_groupped = (mask[knn_idx_flatten, :]).view(n, nsample, 1)
                new_feats_fwd = (new_feats_v * w * mask_groupped).sum(1)
                new_feats_fwd = new_feats_fwd.reshape(-1, self,nsample, sub_nsmaple, out_planes).max(2)[0]
            else:
                new_feats_fwd = (new_feats_v * w).reshape(-1, self.nsample, sub_nsmaple, out_planes).max(2)[0]

            new_feats_fwd = new_feats_fwd.sum(1)

            if use_bwd:
                if sub_nsmaple > 1:
                    energy = energy.reshape(n, self.nsample, sub_nsmaple, -1).reshape(n, self.nsample, -1)
                    energy_bwd_flatten_shrink = rearrange(energy, 'm k c -> (m k) c') # c = 3+out_planes
                    energy_bwd_flatten_shrink = energy_bwd_flatten_shrink.reshape(energy_bwd_flatten_shrink.shape[0], sub_nsmaple, -1)
                else:
                    energy_bwd_flatten_shrink = rearrange(energy, 'm k c -> (m k) c') # c = 3+out_planes
                ...
                if sub_nsmaple > 1:
                    new_feats_v = new_feats_v.reshape(n, self.nsample, sub_nsmaple, -1).reshape(n, self.nsample, -1)
                    new_feats_v_bwd_flatten_shrink = rearrange(new_feats_v, 'm k c -> (m k) c') # c = 3+out_planes
                    new_feats_v_bwd_flatten_shrink = new_feats_v_bwd_flatten_shrink.reshape(new_feats_v_bwd_flatten_shrink.shape[0], sub_nsmaple, -1)
                    new_feats_weighted_flatten = (new_feats_v_bwd_flatten_shrink * energy_bwd_prob_flatten_shrink).max(1)[0]
                else:
                    new_feats_v_bwd_flatten_shrink = rearrange(new_feats_v, 'm k c -> (m k) c') # c = 3+out_planes
                    new_feats_weighted_flatten = new_feats_v_bwd_flatten_shrink * energy_bwd_prob_flatten_shrink
                
                if mask != None and not reverse:
                    # backward cost
    ...
    def forward(self, pc1, pc2, dpc2=None, sf=None, sf_feat=None, mask=None):
        '''
        Input:
        (dense) pc1: [pos:[n, 3]; feats:[n, c]; cnt_num:[b*n]]
        (sparse) pc2: [pos:[m, 3]; feats:[m, c]; cnt_num:[b2*n]]
        ...
        if dpc2 != None:
            dp2, dfeats2, d_cnt_num2 = dpc2
            dp2 = dp2.permute(0,2,1).contiguous().view(-1, dp2.shape[1])
            dfeats2 = dfeats2.permute(0,2,1).contiguous().view(-1, dfeats2.shape[1])
            dpc2 = [dp2, dfeats2, d_cnt_num2]
            dpos_diff_knn, _ = pointops.queryandgroup(
                self.sub_nsample, dp2, p2, dfeats2, None, d_cnt_num2, cnt_num2, use_xyz=True, return_idx=True)
            dpos_diff_knn = dpos_diff_knn[:,:,:3].unsqueeze(1).repeat(1,self.nsample,1,1)
        else:
            dpos_diff_knn = None
        ...
        inter_feats_fwd, inter_feats_bwd, _, _ = self.get_patch_features(pc1, pc2, dpos_diff_knn, channelMixMLPs01=self.channelMixMLPs01, channelMixMLPs02=self.channelMixMLPs02, channelMixMLPs03=self.channelMixMLPs03, linear_p=self.linear_p, sf=sf, use_cross_att=True, reverse=False)

        pc1 = [p1, inter_feats_fwd, cnt_num1]
        pc2 = [p2, inter_feats_bwd, cnt_num2]
        inter_feats_fwd, inter_feats_bwd, _, _ = self.get_patch_features(pc1, pc2, dpos_diff_knn, channelMixMLPs01=self.channelMixMLPs01_2, channelMixMLPs02=self.channelMixMLPs02_2, channelMixMLPs03=self.channelMixMLPs03_2, linear_p=self.linear_p2, sf=sf, use_cross_att=True, reverse=False)

        # patch_to_patch_cost = patch_to_patch_cost.view(B,-1, patch_to_patch_cost.shape[-1]).permute(0,2,1).contiguous()
        inter_feats_fwd = inter_feats_fwd.view(B,-1, inter_feats_fwd.shape[-1]).permute(0,2,1).contiguous()
        inter_feats_bwd = inter_feats_bwd.view(B,-1, inter_feats_bwd.shape[-1]).permute(0,2,1).contiguous()
        return inter_feats_fwd, inter_feats_fwd, inter_feats_bwd
    

class CostVolumeNetFC(nn.Module):
    def __init__(self, nsample, in_channel, mid_channel, share_channel, out_channel, use_leaky=True,
        intraLayer='PointMixerIntraSetLayer',
        interLayer='PointMixerInterSetLayer',
        transup='SymmetricTransitionUpBlock', 
        transdown='TransitionDownBlock'):
        ...
        self.channelMixMLPs_feat_s = nn.Linear(in_channel, out_channel)
        self.channelMixMLPs_feat_s2 = nn.Linear(out_channel, out_channel)

        self.channelMixMLPs_feat_t = nn.Linear(in_channel, out_channel)
        self.channelMixMLPs_feat_t2 = nn.Linear(out_channel, out_channel)
        ...
        # mlp = [out_channel]
        # for out_channel in mlp:
        #     self.mlp_convs.append(nn.Conv2d(last_channel, out_channel, 1, bias = True))
        #     if self.use_bn:
        #         self.mlp_bns.append(nn.BatchNorm2d(out_channel))
        #     last_channel = out_channel
        ...
        # # mlp = [mid_channel, out_channel]
        # for out_channel in mlp:
        #     self.mlp2_convs.append(nn.Conv2d(last_channel, out_channel, 1, bias = True))
        #     if self.use_bn:
        #         self.mlp2_bns.append(nn.BatchNorm2d(out_channel))
        #     last_channel = out_channel
   
        self.channelMixMLPs01 = nn.Sequential( # input.shape = [N, K, C]
                nn.Linear(out_channel, nsample),
                nn.ReLU(inplace=True),
                BilinearFeedForward(nsample, nsample, nsample))
        ...
        self.channelMixMLPs02 = nn.Sequential( # input.shape = [N, K, C]
            Rearrange('n k c -> n c k'),
            nn.Conv1d(out_channel, mid_channel, kernel_size=1, bias=False),
            nn.BatchNorm1d(mid_channel), 
            nn.ReLU(inplace=True),
            nn.Conv1d(mid_channel, mid_channel//share_channel, kernel_size=1, bias=True),
            Rearrange('n c k -> n k c'))
    ...
    def get_patch_features(self, pc1, pc2, bn, channelMixMLPs_feat_s, channelMixMLPs_feat_t, channelMixMLPs01, channelMixMLPs02, channelMixMLPs03, linear_p, sf=None, mask=None, use_cross_att=True, use_bwd=False):
        '''
        Input:
        (dense) pc1: [pos:[n, 3]; feats:[n, c]; cnt_num:[b*n]]
        (sparse) pc2: [pos:[m, 3]; feats:[m, c]; cnt_num:[b2*n]]
        ...
        pos_diff = p2[knn_idx.view(-1), :].view(p1.shape[0], self.nsample, 3) - p1.unsqueeze(1) #pos_feats_knn[:, :, 0:3]
        # pos_diff_feat = channelMixMLPs_feat_s(pos_diff)
        p_embed = linear_p(pos_diff) # (n, k, out_planes)
        p_embed_shrink = self.shrink_p(p_embed) # (n, k, k)
        
        pos_feats_knn[:,:,:3] = pos_diff
        new_feats = torch.cat([pos_feats_knn, feats1.view(len(p1), 1, -1).repeat(1, self.nsample, 1)], dim=-1)
        
        # pos_feats1_knn = torch.cat([pos_diff, feats1_groupped], dim=-1)
        feats1 = channelMixMLPs_feat_s(feats1)
        feats1_groupped = feats1.view(len(p1), 1, -1).repeat(1, self.nsample, 1)
        pos_feats1_knn= p_embed + feats1_groupped
        pos_feats1_knn = pos_feats1_knn.unsqueeze(2).repeat(1,1,self.nsample,1)
        
        pos_feats2_knn = channelMixMLPs_feat_t(pos_feats_knn[:,:,3:])
        pos_feats2_knn = pos_feats2_knn.unsqueeze(1).repeat(1,self.nsample,1,1)
        new_feats_expand = pos_feats1_knn + pos_feats2_knn            
        energy = torch.max(new_feats_expand, dim=2)[0]
    ...
    def forward(self, pc1, pc2, sf=None, mask=None):
        ...
        inter_feats_fwd, inter_feats_bwd, _, _ = self.get_patch_features(pc1, pc2, bn=self.bn, channelMixMLPs_feat_s=self.channelMixMLPs_feat_s, channelMixMLPs_feat_t=self.channelMixMLPs_feat_t, channelMixMLPs01=self.channelMixMLPs01, channelMixMLPs02=self.channelMixMLPs02, channelMixMLPs03=self.channelMixMLPs03, linear_p=self.linear_p, sf=sf, use_cross_att=True, use_bwd=True)

        pc1 = [p1, inter_feats_fwd, cnt_num1]
        pc2 = [p2, inter_feats_bwd, cnt_num2]
        inter_feats_fwd, inter_feats_bwd, _, _ = self.get_patch_features(pc1, pc2, bn=self.bn2, channelMixMLPs_feat_s=self.channelMixMLPs_feat_s2, channelMixMLPs_feat_t=self.channelMixMLPs_feat_t2, channelMixMLPs01=self.channelMixMLPs01_2, channelMixMLPs02=self.channelMixMLPs02_2, channelMixMLPs03=self.channelMixMLPs03_2, linear_p=self.linear_p2, sf=sf, use_cross_att=True, use_bwd=True)
...
class CostVolumeNet(nn.Module):
    def __init__(self, nsample, in_channel,  sf_channel, mid_channel, share_channel, out_channel, use_mask=False, use_leaky=True,
        intraLayer='PointMixerIntraSetLayer',
        interLayer='PointMixerInterSetLayer',
        transup='SymmetricTransitionUpBlock', 
        transdown='TransitionDownBlock'):
        ...
        self.channelMixMLPs01 = nn.Sequential( # input.shape = [N, K, C]
                nn.Linear(in_channel*2+3+sf_channel, nsample),
                nn.ReLU(inplace=True),
                BilinearFeedForward(nsample, nsample, nsample))
        
        self.channelMixMLPs01_2 = nn.Sequential( # input.shape = [N, K, C]
                nn.Linear(out_channel*2+3+sf_channel, nsample),
                nn.ReLU(inplace=True),
                BilinearFeedForward(nsample, nsample, nsample))
        
        self.linear_p = nn.Sequential( # input.shape = [N, K, C]
            nn.Linear(3+sf_channel, out_channel//2, bias=False),
            nn.Sequential(
                Rearrange('n k c -> n c k'),
                nn.BatchNorm1d(out_channel//2),
                Rearrange('n c k -> n k c')),
        ...
        self.channelMixMLPs03 = nn.Linear(in_channel*2+3+sf_channel, out_channel)
        self.channelMixMLPs03_2 = nn.Linear(out_channel*2+3+sf_channel, out_channel)
    ...
    def get_patch_features(self, pc1, pc2, channelMixMLPs01, channelMixMLPs02, channelMixMLPs03, linear_p, sf=None, mask=None, use_cross_att=True, use_bwd=False):
        '''
        Input:
        (dense) pc1: [pos:[n, 3]; feats:[n, c]; cnt_num:[b*n]]
        (sparse) pc2: [pos:[m, 3]; feats:[m, c]; cnt_num:[b2*n]]
        ...
        feats1_groupped = feats1.view(len(p1), 1, -1).repeat(1, self.nsample, 1)
        if sf != None:
            sf_groupped = sf.view(len(p1), 1, -1).repeat(1, self.nsample, 1)
            new_feats = torch.cat([pos_feats_knn, feats1_groupped, sf_groupped], dim=-1)
            pos_diff = torch.cat([pos_diff, sf_groupped], dim=-1)
        else:
            new_feats = torch.cat([pos_feats_knn, feats1_groupped], dim=-1)
    ...
    def forward(self, pc1, pc2, sf=None, sf_feat=None, mask=None):
        ...
        inter_feats_fwd, inter_feats_bwd, _, _ = self.get_patch_features(pc1, pc2, channelMixMLPs01=self.channelMixMLPs01, channelMixMLPs02=self.channelMixMLPs02, channelMixMLPs03=self.channelMixMLPs03, linear_p=self.linear_p, sf=sf, mask=mask, use_cross_att=True, use_bwd=True)
        ...
        pc1 = [p1, inter_feats_fwd, cnt_num1]
        pc2 = [p2, inter_feats_bwd, cnt_num2]
        inter_feats_fwd, inter_feats_bwd, _, _ = self.get_patch_features(pc1, pc2, channelMixMLPs01=self.channelMixMLPs01_2, channelMixMLPs02=self.channelMixMLPs02_2, channelMixMLPs03=self.channelMixMLPs03_2,linear_p=self.linear_p2, sf=sf, mask=mask, use_cross_att=True, use_bwd=True)
        
        # patch_to_patch_cost = patch_to_patch_cost.view(B,-1, patch_to_patch_cost.shape[-1]).permute(0,2,1).contiguous()
        inter_feats_fwd = inter_feats_fwd.view(B,-1, inter_feats_fwd.shape[-1]).permute(0,2,1).contiguous()
        inter_feats_bwd = inter_feats_bwd.view(B,-1, inter_feats_bwd.shape[-1]).permute(0,2,1).contiguous()
        return inter_feats_fwd, inter_feats_bwd
...
class UpsampleFlow(nn.Module):
    def forward(self, xyz, sparse_xyz, sparse_flow):
        ...
        dist, knn_idx = pointutils.knn(3, xyz, sparse_xyz)
...
class CostVolumeNetMS(nn.Module):
    def __init__(self, nsample, in_channel,  in_channel2, mid_channel, share_channel, out_channel, mlp=[], mlp2=[], use_mask=False, use_leaky=True,
        intraLayer='PointMixerIntraSetLayer',
        interLayer='PointMixerInterSetLayer',
        transup='SymmetricTransitionUpBlock', 
        transdown='TransitionDownBlock'):
        ...
        self.channelMixMLPs01_2 = nn.Sequential( # input.shape = [N, K, C]
                nn.Linear(out_channel+3+ out_channel//share_channel+in_channel2, nsample),
                nn.ReLU(inplace=True),
                BilinearFeedForward(nsample, nsample, nsample))
        ...
        self.channelMixMLPs02 = nn.Sequential( # input.shape = [N, K, C]
            ...
            nn.BatchNorm1d(mid_channel), 
            nn.ReLU(inplace=True),
            nn.Conv1d(mid_channel, out_channel, kernel_size=1, bias=True),
            Rearrange('n c k -> n k c'))
        ...
        self.channelMixMLPs03 = nn.Linear(in_channel*2+3, out_channel)
        self.channelMixMLPs03_2 = nn.Linear(out_channel + 3 + out_channel//share_channel+in_channel2, out_channel)
        ...
        self.deconv = Conv1d(out_channel, out_channel//share_channel)
        # self.deconv_2 = Conv1d(in_channel2 + out_channel//share_channel, out_channel)
        
        self.ds_mlp_convs = nn.ModuleList()
        self.ds_mlp_bns = nn.ModuleList()
        self.ds_mlp2_convs = nn.ModuleList()
        last_channel = out_channel + 3
        for out_channel in mlp:
            self.ds_mlp_convs.append(nn.Conv2d(last_channel, out_channel, 1, bias = False))
            self.ds_mlp_bns.append(nn.BatchNorm2d(out_channel))
            last_channel = out_channel
        if mlp2 != None:
            for out_channel in mlp2:
                self.ds_mlp2_convs.append(nn.Sequential(nn.Conv1d(last_channel, out_channel, 1, bias=False),
                                                    nn.BatchNorm1d(out_channel)))
                last_channel = out_channel
        
    
    def get_patch_features(self, pc1, pc2, channelMixMLPs01, channelMixMLPs02, channelMixMLPs03, sf=None, mask=None, use_cross_att=True, use_bwd=False):
        '''
        Input:
        (dense) pc1: [pos:[n, 3]; feats:[n, c]; cnt_num:[b*n]]
        (sparse) pc2: [pos:[m, 3]; feats:[m, c]; cnt_num:[b2*n]]
        ...
        if use_cross_att: # and sf != None
            ...
            if use_bwd:
                ...
                new_feats_bwd = scatter_sum(new_feats_weighted_flatten, knn_idx_flatten, dim=0, dim_size=len(p2))
    ...
    def downsample_features(self, pc, spc, feats, sfeats, ds_mlp_convs, ds_mlp_bns, ds_mlp2_convs):   
        # feats_t = feats.view(B,-1, feats.shape[-1]).contiguous()
        new_points, _ = group_query(self.nsample, pc, spc, feats) # [B, N, S, 3+C]
        new_points = new_points.permute(0,3,1,2).contiguous()
        for i, conv in enumerate(ds_mlp_convs):
            bn = ds_mlp_bns[i]
            new_points =  F.relu(bn(conv(new_points)))
        
        new_points = torch.max(new_points, -1)[0]

        for i, conv in enumerate(ds_mlp2_convs):
            new_points = F.relu(conv(new_points)) #[B,C,N]
            
        return new_points
          
            
    def forward(self, pc1, pc2, dpc2, sf=None, sf_feat=None, mask=None):
        '''
        Input:
        (dense) dpc2: [pos:[n, 3]; feats:[n, c]; cnt_num:[b*n]]
        (sparse) pc1,pc2: [pos:[m, 3]; feats:[m, c]; cnt_num:[b2*n]]
        s_sf: [n,3], cnt_num: [b]
        '''
        p1, feats1, cnt_num1 = pc1
        p2, feats2, cnt_num2 = pc2 
        ...
        if len(p1.shape) == 3:
            ...
            pc1 = [p1, feats1, cnt_num1]
        if len(p2.shape) == 3:
            p2_flatten = p2.permute(0,2,1).contiguous().view(-1, p2.shape[1])
            feats2 = feats2.permute(0,2,1).contiguous().view(-1, feats2.shape[1])
            pc2 = [p2_flatten, feats2, cnt_num2]
        ...
        inter_feats_fwd, inter_feats_bwd, _, _ = self.get_patch_features(pc1, pc2, channelMixMLPs01=self.channelMixMLPs01, channelMixMLPs02=self.channelMixMLPs02, channelMixMLPs03=self.channelMixMLPs03, sf=sf, mask=mask, use_cross_att=True, use_bwd=True)
        
        if len(dp2.shape) == 3:
            dp2_flatten = dp2.permute(0,2,1).contiguous().view(-1, dp2.shape[1])
            up_inter_feat_bwd = self.upsample(dp2, p2, inter_feats_bwd.view(B,-1, inter_feats_bwd.shape[-1]).permute(0,2,1))
            
            up_inter_feat_bwd = self.deconv(up_inter_feat_bwd)
            up_inter_feat_bwd = torch.cat([dfeats2, up_inter_feat_bwd], dim=1)
            # up_inter_feat_bwd = self.deconv_2(up_inter_feat_bwd)
            
            up_inter_feat_bwd = up_inter_feat_bwd.permute(0,2,1).contiguous().view(-1, up_inter_feat_bwd.shape[1])
            
            dpc2 = [dp2_flatten, up_inter_feat_bwd, d_cnt_num2]
            
            dp2_t = dp2.permute(0,2,1).contiguous()
            
        pc1 = [p1, inter_feats_fwd, cnt_num1]
        
        d_inter_feats_fwd, d_inter_feats_bwd, _, _ = self.get_patch_features(pc1, dpc2, channelMixMLPs01=self.channelMixMLPs01_2, channelMixMLPs02=self.channelMixMLPs02_2, channelMixMLPs03=self.channelMixMLPs03_2, sf=sf, use_cross_att=True, use_bwd=True)
        
        d_inter_feats_bwd_t = d_inter_feats_bwd.view(B,-1, d_inter_feats_bwd.shape[-1]).contiguous()
        new_d_inter_feats_bwd = self.downsample_features(dp2_t, p2_t, d_inter_feats_bwd_t, inter_feats_bwd, self.ds_mlp_convs, self.ds_mlp_bns, self.ds_mlp2_convs)
        ...
        # patch_to_patch_cost = patch_to_patch_cost.view(B,-1, patch_to_patch_cost.shape[-1]).permute(0,2,1).contiguous()
        inter_feats_fwd = d_inter_feats_fwd.view(B,-1, d_inter_feats_fwd.shape[-1]).permute(0,2,1).contiguous()
        d_inter_feats_bwd = d_inter_feats_bwd.view(B,-1, d_inter_feats_bwd.shape[-1]).permute(0,2,1).contiguous()
        return inter_feats_fwd, new_d_inter_feats_bwd, d_inter_feats_bwd


class CostVolumeNetMS2(nn.Module):
    def __init__(self, nsample, in_channel,  in_channel2, mid_channel, share_channel, out_channel, mlp=[], mlp2=[], use_mask=False, use_leaky=True,
        intraLayer='PointMixerIntraSetLayer',
        interLayer='PointMixerInterSetLayer',
        transup='SymmetricTransitionUpBlock', 
        ...
        self.channelMixMLPs01_2 = nn.Sequential( # input.shape = [N, K, C]
                nn.Linear(out_channel*2+3+ out_channel//share_channel, nsample),
                nn.ReLU(inplace=True),
                BilinearFeedForward(nsample, nsample, nsample))
        ...
        self.channelMixMLPs03 = nn.Linear(in_channel*2+3, out_channel)
        self.channelMixMLPs03_2 = nn.Linear(out_channel * 2 + 3 + out_channel//share_channel, out_channel)
...
class OcclusisonEstiamtionNet(nn.Module):
    def __init__(self, nsample, voxel_size, resolution, in_channels, num_levels=2, out_channel=256, transup='SymmetricTransitionUpBlock'):
        ...
        for i, in_channel in enumerate(in_channels):
            self.TransitionUpNetlayers.append(SymmetricTransitionUpBlock(in_channel, out_channel, nsample=16))
...
class SceneFlowUpsampleNet(nn.Module):
    def __init__(self, nsample, num_levels, voxel_size, resolution, sf_channel, in_channel, hidden_channel, out_channel, use_voxel_mixer=True,
        ...
        # last_channel = in_channel + sf_channel
        last_channel = sf_channel
    ...
    def forward(self, pc1, pc2, s_pc1, sf, sf_feats, mask=None):
        ...
        if B != None:
            up_sf_feats = up_sf_feats.view(B, -1, up_sf_feats.shape[-1]).permute(0,2,1).contiguous() #None #
...
class SceneFlowRegressor(nn.Module):
    def __init__(self, nsample, in_channel,  sfeat_channel,  sf_channel, mid_channel, share_channel, out_channel, channels=[128,128], mlp=[128, 128], use_mask=False, use_leaky=True,
        ...
        self.sfnet = SceneFlowEstimatorPointConv(feat_ch=sfeat_channel, cost_ch=out_channel, flow_ch=sf_channel, channels =channels, mlp=mlp, share_planes=8, neighbors=8, clamp=[-200, 200], use_leaky = True)
        self.sfnet2 = SceneFlowEstimatorPointConv(feat_ch=sfeat_channel, cost_ch=out_channel, flow_ch=3, channels =channels, mlp=mlp, share_planes=8, neighbors=8, clamp=[-200, 200], use_leaky = True)
...
def group(nsample, xyz, points, mask=None):
    ...
    _, idx  = pointutils.knn(nsample, new_xyz, xyz)
...
class SceneFlowEstimatorResidual(nn.Module):
    ...
    def forward(self, xyz, feats, cost_volume, flow = None, mask=None, recurrent_mode=False):
        '''
        feats: B C1 N
        cost_volume: B C2 N
        flow: B 3 N
        ...
        if recurrent_mode:
            cost_volume = cost_volume.view(B,-1, cost_volume.shape[-1]).permute(0,2,1).contiguous()
            flow = flow.view(B,-1, 3).permute(0,2,1).contiguous()
        ...
        for _, pointconv in enumerate(self.pointconv_list):
            new_points = pointconv(xyz, new_points, mask)
        ...
        if recurrent_mode:
            flow = flow.permute(0,2,1).reshape(-1,3)
